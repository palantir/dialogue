...
final class DialogueDnsResolution {
    ...
    static RefreshableUris refreshableUris(String channelName, ClientConfiguration configuration) {
        ImmutableList<TargetUri> initial = resolveUris(configuration);
        ...
        ScheduledFuture<?> dnsRefreshFuture = SINGLETON_SCHEDULER.scheduleWithFixedDelay(
                () -> {
                    try {
                        refreshable.update(resolveUris(configuration));
                    } catch (Throwable t) {
                ...
                5,
                TimeUnit.SECONDS);
        return new RefreshableUris(refreshable, dnsRefreshFuture);
    }
    ...
    static final class RefreshableUris implements Closeable {
        ...
        private RefreshableUris(Refreshable<ImmutableList<TargetUri>> uris, ScheduledFuture<?> dnsRefreshFuture) {
            this.uris = uris;
        ...
        @Override
        public void close() {
            dnsRefreshFuture.cancel(false);
        }
    }

    static ImmutableList<TargetUri> resolveUris(ClientConfiguration configuration) {
        ...
        for (String uri : uris) {
            ...
            if (isInetAddress || usesProxy) {
                ...
                try {
                    ...
                    log.warn("DNS Resolution failed", e);

...
public class DialogueDnsResolutionWorker implements Runnable {
    ...
    public DialogueDnsResolutionWorker(SettableRefreshable<ServicesConfigBlockWithResolvedHosts> receiver) {
    ...
    public void run() {
        while (!shutdownRequested) {
            try {
                ...
                if (inputState != null) {
                    ImmutableSetMultimap<String, TargetUri> resolvedUris =
                            ServicesConfigBlockWithResolvedHosts.from(inputState);
                    ServicesConfigBlockWithResolvedHosts newResolvedState =
                            new ServicesConfigBlockWithResolvedHosts(inputState, resolvedUris);

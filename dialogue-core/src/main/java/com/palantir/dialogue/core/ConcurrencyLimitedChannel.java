...
final class ConcurrencyLimitedChannel implements LimitedChannel {
        ...
        switch (clientQoS) {
            case ENABLED:
                ...
                return new ConcurrencyLimitedChannel(
                        channel, createLimiter(cf.ticker()), cf.channelName(), uriIndex, metrics);
    ...
    public Optional<ListenableFuture<Response>> maybeExecute(Endpoint endpoint, Request request) {
        ...
        if (maybeListener.isPresent()) {
            ...
            log.info("Limited. {}", SafeArg.of("max", limiter.getLimit()));
            limitedMeter.mark();
     ...
     */
    private final class LimiterCallback implements FutureCallback<Response> {
        ...
        public void onSuccess(Response result) {
            if (Responses.isQosStatus(result) || Responses.isServerError(result)) {
                listener.onDropped();
                log.info("DOWN {}", SafeArg.of("newLimit", limiter.getLimit()));
            } else {
                listener.onSuccess();
                log.info("UP {}", SafeArg.of("newLimit", limiter.getLimit()));
            }
        }

...
class RequestFlowWalkthroughTest {
    ...
    // ========================================================================================
    //  Queue timeout: periodic sweep fails expired entries, scheduleNextTask cleans them up
    // ========================================================================================
    ...
    void outer_queue_timeout_evicts_requests() throws Exception {
        ...
        // Wait for the periodic sweep to fire (sweep interval = timeout / 2 = 50ms,
        // so after 200ms the sweep has run multiple times and all entries are expired)
        Thread.sleep(queueTimeout.toMillis() + 100);

        // The sweep should have called setException on all 5 futures
        for (int i = 0; i < 5; i++) {
            assertThat(queued[i])
                    .as("Queued request %d should be done (timed out by sweep)", i)
                    .isDone();
            assertThat(queued[i]).failsWithin(Duration.ZERO).withThrowableThat().withMessageContaining("queue timeout");
        }

        // The main deque entries are still present (sweep doesn't remove them).
        // Complete one wire request to trigger scheduleNextTask, which will see isDone()
        // and clean up the entries (decrement queue size, complete spans, stop timers).
        completeOne(HOST_A);
        assertThat(outerQueue.getQueueSizeForTesting())

...
public class BlacklistingChannelTest {
    ...
    @Mock
    private CompositeLimitedChannel delegate;
    ...
    public void before() {
        ...
        when(delegate.maybeExecute(endpoint, request)).thenReturn(LimitedResponses.response(futureResponse));
    }

    @Test
    public void testBlacklistAfterError() {
        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(futureResponse);
    ...
    public void after_blacklisting_expires_only_5_requests_are_allowed_to_start() {
        ...
        when(delegate.maybeExecute(endpoint, request)).thenReturn(LimitedResponses.response(unresolvedResponse));
        when(ticker.read()).thenReturn(BLACKLIST_DURATION.toNanos());

        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(unresolvedResponse);
        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(unresolvedResponse);
        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(unresolvedResponse);
    ...
    public void after_blacklisting_expires_and_5_requests_pass_we_are_good_to_go_again() {
        ...
        when(delegate.maybeExecute(endpoint, request)).thenReturn(LimitedResponses.response(thisWillSucceed));
        when(ticker.read()).thenReturn(BLACKLIST_DURATION.toNanos());

        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(thisWillSucceed);
        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(thisWillSucceed);
        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(thisWillSucceed);
    ...
    public void failure_during_probation_puts_us_back_into_blacklisting() {
        ...
        when(delegate.maybeExecute(endpoint, request)).thenReturn(LimitedResponses.response(thisWill503));
        when(ticker.read()).thenReturn(BLACKLIST_DURATION.toNanos());

        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(thisWill503);
        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(thisWill503);
        assertThat(LimitedResponses.getResponse(channel.maybeExecute(endpoint, request)))
                .contains(thisWill503);

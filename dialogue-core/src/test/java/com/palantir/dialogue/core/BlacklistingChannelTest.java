...
public class BlacklistingChannelTest {
    ...
    public void before() {
        ...
        when(delegate.maybeExecute(request)).thenReturn(Optional.of(futureResponse));
    }

    @Test
    public void testBlacklistAfterError() {
        assertThat(channel.maybeExecute(request)).contains(futureResponse);

        futureResponse.setException(new IllegalStateException());
        assertThat(channel.maybeExecute(request)).isEmpty();
    }
    ...
    public void after_blacklisting_expires_only_5_requests_are_allowed_to_start() {
        ...
        when(delegate.maybeExecute(request)).thenReturn(Optional.of(unresolvedResponse));
        when(ticker.read()).thenReturn(BLACKLIST_DURATION.toNanos());

        assertThat(channel.maybeExecute(request)).contains(unresolvedResponse);
        assertThat(channel.maybeExecute(request)).contains(unresolvedResponse);
        assertThat(channel.maybeExecute(request)).contains(unresolvedResponse);
        assertThat(channel.maybeExecute(request)).contains(unresolvedResponse);
        assertThat(channel.maybeExecute(request)).contains(unresolvedResponse);
    ...
    public void after_blacklisting_expires_and_5_requests_pass_we_are_good_to_go_again() {
        ...
        when(delegate.maybeExecute(request)).thenReturn(Optional.of(thisWillSucceed));
        when(ticker.read()).thenReturn(BLACKLIST_DURATION.toNanos());

        assertThat(channel.maybeExecute(request)).contains(thisWillSucceed);
        assertThat(channel.maybeExecute(request)).contains(thisWillSucceed);
        assertThat(channel.maybeExecute(request)).contains(thisWillSucceed);
        assertThat(channel.maybeExecute(request)).contains(thisWillSucceed);
        assertThat(channel.maybeExecute(request)).contains(thisWillSucceed);
        ...
        assertThat(channel.maybeExecute(request)).isPresent();
    }
    ...
    public void failure_during_probation_puts_us_back_into_blacklisting() {
        ...
        when(delegate.maybeExecute(request)).thenReturn(Optional.of(thisWill503));
        when(ticker.read()).thenReturn(BLACKLIST_DURATION.toNanos());

        assertThat(channel.maybeExecute(request)).contains(thisWill503);
        assertThat(channel.maybeExecute(request)).contains(thisWill503);
        assertThat(channel.maybeExecute(request)).contains(thisWill503);
        assertThat(channel.maybeExecute(request)).contains(thisWill503);
        assertThat(channel.maybeExecute(request)).contains(thisWill503);
